...
#Ideally you are running the latest version of AzureRM commands, minimum version is 6.0
$TestAzure = (get-installedmodule -name azurerm -MinimumVersion 6.0)
If ($TestAzure)
        {
        Import-Module -Name AzureRM
        }
Else {Write-Host "Update your Azure Powershell Module" -ForegroundColor Red
      Break }

Add-AzureRmAccount

$AzureSub = (Get-AzureRmSubscription | Out-GridView -OutputMode Single -Title 'Select the Subscription').id

Select-AzureRmSubscription -Subscription $AzureSub

#Added this function to allow parallel execution of the script across all VMSS instances
...
#Added this function to allow you to pick the script you want to run, either a PowerShell or Bash script works.
...
#Getting the script details and adding it to a variable to pass to the invoke command
$inputfile = Get-FileName "C:\"
$inputdata = get-content $inputfile

#If the file selected is a PowerShell script then this is the parameters to use
$PowershellScript =
@{
 ...
 #IF the file selected is a Bash script then this is the parameters to use
$ShellScript =
@{
...
#Creating an array to save all the instances to
$LinuxList = New-Object System.Collections.ArrayList
$WindowsList = New-Object System.Collections.ArrayList
$request = New-Object System.Collections.ArrayList

#Loop thru the lists provided and execute the file on the VMSS instances
foreach ($Instance in $VMSSInstance)
{
 
    $VMSSIndVM = Get-AzureRmVmssVM -ResourceGroupName $VMSS.ResourceGroupName -VMScaleSetName $VMSS.Name -InstanceId $Instance
    $OSType = $VMSSIndVM.StorageProfile.OsDisk.OsType
    $VMSSID = $VMSSIndVM.Id
    If ($OSType -eq "Linux"-and ([System.IO.Path]::GetExtension($inputfile) -eq ".sh")){
        [void]$LinuxList.Add($VMSSID)
        }
    If ($OSType -eq "Windows" -and ([System.IO.Path]::GetExtension($inputfile) -eq ".ps1")){
         [void]$WindowsList.Add($VMSSID)
        }
...
else {Write-Host "Check your Script against VMSS OS Type" -ForegroundColor red}

#Get the results from the script execution and present it back
$request | Format-Table -Property * -wrap -AutoSize 
